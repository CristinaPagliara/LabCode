    ...
    if ~isempty(area_spikes)
        ...
        for unit = 1:size(area_spikes,1) %units in the IdxArea for each mouse
            ...
            if h == 1 
                ...
                significant_delta = [significant_delta abs(significant)];
...
%% 2 striatum visual fast contra - among the units with a potentiation, how many have tactile and visual properties? 
rng(1);
...
for idxMouse  = 1:size(mouse,2)
    ...
    if ~isempty(area_spikes)
        farON_PRE = timestamps.Ton((timestamps.sideIdx == 1) & (timestamps.endloc == 1) & (timestamps.speeds == 2) & (timestamps.phase == 1) );
        farOFF_PRE = timestamps.Toff((timestamps.sideIdx == 1) & (timestamps.endloc == 1)  & (timestamps.speeds == 2) & (timestamps.phase == 1));
        farON_POST = timestamps.Ton((timestamps.sideIdx == 1) & (timestamps.endloc == 1) & (timestamps.speeds == 2) & (timestamps.phase == 3));
        farOFF_POST = timestamps.Toff((timestamps.sideIdx == 1) & (timestamps.endloc == 1) & (timestamps.speeds == 2) & (timestamps.phase == 3) & (timestamps.isDarkTrial == 0));
        ssON = timestamps.Ton((timestamps.sideIdx == 1) & (timestamps.endloc == 3) & (timestamps.speeds == 2) & (timestamps.phase == 2));
        ssOFF = timestamps.Toff((timestamps.sideIdx == 1) & (timestamps.endloc == 3) & (timestamps.speeds == 2) & (timestamps.phase == 2));
        visON_PRE = timestamps.Ton((timestamps.sideIdx == 1) & (timestamps.endloc == 1) & (timestamps.speeds == 2) & (timestamps.phase == 1) & (timestamps.isDarkTrial == 0));
        visOFF_PRE = timestamps.Toff((timestamps.sideIdx == 1) & (timestamps.endloc == 1)  & (timestamps.speeds == 2) & (timestamps.phase == 1) & (timestamps.isDarkTrial == 0));
        ...
        for unit = 1:size(area_spikes,1) %units in the IdxArea for each mouse
            ...
            countsRsp = [];
            deltaPRE = [];
            for idxTrial = 1:length(farON_PRE)
                % Trial_number= find(ismember(timestamps.Toff, ssOFF(idxTrial)));
                window = min(timestamps.Toff-timestamps.Ton);
                countsBsl(idxTrial) = CountInIntervals(spikes,[farON_PRE(idxTrial)-window farON_PRE(idxTrial)]);
                countsRsp(idxTrial) = CountInIntervals(spikes,[farON_PRE(idxTrial) farON_PRE(idxTrial)+window]);
                deltaPRE(idxTrial) = countsRsp(idxTrial) - countsBsl(idxTrial);
            end
            ...
            countsRsp = [];
            deltaPOST = [];
            for idxTrial = 1:length(farON_POST)
                % Trial_number= find(ismember(timestamps.Toff, ssOFF(idxTrial)));
                window = min(timestamps.Toff-timestamps.Ton);
                countsBsl(idxTrial) = CountInIntervals(spikes,[farON_POST(idxTrial)-window farON_POST(idxTrial)]);
                countsRsp(idxTrial) = CountInIntervals(spikes,[farON_POST(idxTrial) farON_POST(idxTrial)+window]);
                deltaPOST(idxTrial) = countsRsp(idxTrial) - countsBsl(idxTrial);
            end

            [auROC, significant] = findAuROC(deltaPRE, deltaPOST, 1, 1000);
            auROC_delta = [auROC_delta auROC];
            significant_delta = [significant_delta abs(significant)];

            if significant_delta (unit) == 1
...
potent_units=(sum(significant_delta) ./ tot_unit) * 100
